...
def check(row, col, num, queue, count_map, path):
...
def part1(row, col):
    ...
    while len(queue) > 0:
        ...
        check(row, col, num, queue, count_map, path)
    
    if count_map[target_row][target_col] == 0:
        return 100000000
    else:
        return count_map[target_row][target_col]

answer = 100000000
for row in range(len(maze)):
    for col in range(len(maze[0])):
        if maze[row][col] == 'a':
            answer = min(answer, part1(row, col))
